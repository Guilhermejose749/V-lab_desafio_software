...
def test_edit_course_as_owner(client, owner_token, setup_course):
    ...
    response = client.patch(f"/courses/{setup_course}", json=payload, headers=owner_token)
...
def test_edit_course_as_intruder(client, intruder_token, setup_course):
    ...
    response = client.patch(f"/courses/{setup_course}", json=payload, headers=intruder_token)
    # Intruso não tem permissão
    assert response.status_code in [401, 403] 

def test_delete_course_as_intruder(client, intruder_token, setup_course):
    response = client.delete(f"/courses/{setup_course}", headers=intruder_token)
...
def test_delete_course_as_owner(client, owner_token, setup_course):
    ...
    response = client.delete(f"/courses/{setup_course}", headers=owner_token)
    assert response.status_code in [200, 204]
    
    # Verifica se realmente sumiu
    get_response = client.get(f"/courses/{setup_course}")
